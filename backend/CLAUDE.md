...
├── queue/                      # material-analysis（已激活：素材上传异步解析）/ video-generation（空 TODO）
...
**商品主图解析路径（同步；⚠️ 当前仍会顺带造一条素材）：**
```
POST /api/products/:project_id/parse-image  （multipart 商品图）
  └─ ProductService.parseImage(): minio 落盘 + volcano.analyzeProductImage()（同步）
        ├─ 更新 projects.product_info + cover_url
        └─ materialRepo.save(status='ready')   ← ⚠️ 目前仍会建一条素材记录
```
> 解耦待办：理想形态是 parse-image 只写 `product_info`、不造素材，素材统一走 `/api/materials/upload`。当前**尚未解耦**（product 代码暂不动），所以素材库里会混入商品主图。
...
**✅ 已完成（原 Roadmap 项）**：真实 Doubao Vision/Embedding/Seedance 调用、MinIO 真实存储、`Material` 实体/SQL 补齐 `status`/`thumbnail_url`/`duration`/`slices` 列（漂移已修）、**`POST /api/materials/upload` 图片素材上传 + `material-analysis` 队列异步解析（已激活）**。
