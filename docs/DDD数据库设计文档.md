...
```sql
-- 因子替换：追加历史 + 更新 storyboard 中的因子快照
UPDATE scripts
SET storyboard = jsonb_set(
    ...
    factor_history = factor_history || jsonb_build_object(
        'dimension', '视觉风格',
        'old_value', storyboard #>> '{factors,视觉风格}',
        'new_value', $2,
        'timestamp', NOW()
    ),
    updated_at = NOW()
WHERE id = $1
RETURNING storyboard, factor_history;
```
...
| --- | --- | --- | --- | --- | --- |
| `id` | `UUID` | ❌ | `uuid_generate_v4()` | **PK** | — |
| `dimension` | `VARCHAR(100)` | ❌ | — | — | 维度名：视觉风格 / 开场手法 / 旁白风格 / 节奏密度 / CTA 形式 |
...
| `resolution` | `VARCHAR(20)` | ✅ | NULL | 枚举：见 6.4 | `1080P` / `720P` / `1080P_9:16` 等 |
| `status` | `VARCHAR(20)` | ❌ | `'pending'` | 枚举：见 6.4 | `pending`/`generating`/`composing`/`completed`/`failed` |
...
> **业务定位**：单个视频拆出的分镜级 Seedance 任务，是 BullMQ ↔ Seedance API ↔ 回调端点之间的数据载体。
...
商品解析结果。AI（Doubao Function Calling）或人工填写。

```jsonc
{
  "品名":       "雪肌透光亮肤精华液",          // String, required
  "品类":       "美妆护肤",                  // String, enum 见 6.1 品类枚举
  "核心卖点":   ["28天淡化黑色素", "..."],    // String[], 1..5
  "目标人群":   "25-35岁都市女性",            // String, optional
  "使用场景":   "早晚洁面后使用",              // String, optional
  "价格锚点":   "原价¥199，现¥89"             // String, optional
}
```

> 字段名采用中文以与 Doubao 提示词输出对齐，无需后端二次映射。

## 3.2 `scripts.storyboard`

分镜数组 + 因子快照（顶层是对象，含 `shots` 与 `factors`，**当前 seed 文件存的是 shots 数组本身**——v1.1 将统一为对象形式以承载顶层因子快照）。建议形态：
...
```jsonc
{
  ...
  "shots": [
    {
      "shot_index":  1,                          // Integer, ≥ 1，连续递增
      "description": "暗沉肤色特写...",           // String，画面描述
      "camera":      "微距推近",                  // String，镜头运动
      "duration":    3,                          // Number, 秒，1..5
      "narration":   "你是不是也试过...",         // String，配音文案
      "subtitles":   "暗沉困扰？",                // String，字幕
...
> 总分镜 `SUM(duration)` ≤ 15s（应用层校验）。`material_slice_id` 字段在 v1.1 引入，绑定具体素材切片。
...
[
  {
    ...
    "trace_id":  "trace-xxxx"  // 关联本次 Director Agent 调用
  }
]
```
...
某维度的可选值数组（字符串列表）。示例（`视觉风格` 维度）：

```json
["黑风极简","夏日度假风","赛博科技风","轻奢质感风","清新自然风","复古胶片风","高级ins风","温柔奶油风"]
```

> 五个维度的取值见 6.8，禁止与维度无关的字符串混入。
...
```jsonc
{
  ...
  "key_factors": {                                // Object，主导因子（与 3.2 因子结构对齐）
    "视觉风格": "轻奢质感风",
    "开场手法": "问题式Hook",
    "节奏密度": "中速"
  }
...
```jsonc
{
  "hook_type":        "问题式Hook",                          // String，与因子库取值对齐
  "hook_description": "开头直接展示...",                      // String，<= 200
  "shot_count":       5,                                    // Integer
  "rhythm":           "中速1-2s/镜",                         // String
...
| --- | --- |
| `pending` | 已创建，未提交 |
| `generating` | Seedance 任务执行中 |
| `composing` | 进入 FFmpeg 合成 |
| `completed` | 合成完成，已写入 `video_url` |
| `failed` | 全链路任一阶段失败（错误细节见 `video_tasks.error_msg` 或 trace） |

`resolution`：`720P` / `1080P`，可选附加画幅后缀 `_9:16` / `_16:9` / `_1:1`。
...
| 维度 | 可选值 |
| --- | --- |
| 视觉风格 | 黑风极简 / 夏日度假风 / 赛博科技风 / 轻奢质感风 / 清新自然风 / 复古胶片风 / 高级 ins 风 / 温柔奶油风 |
| 开场手法 | 问题式 Hook / 价格锚点 Hook / 悬念式 Hook / 数据震撼 Hook / 对比式 Hook / 情感共鸣 Hook / 反常识 Hook / 明星同款 Hook |
| 旁白风格 | 优雅知性 / 活泼种草 / 专业测评 / 亲切日常 / 温柔故事感 / 激情带货 / 幽默段子手 |
| 节奏密度 | 快切节奏（0.5–1s/镜）/ 中速（1–2s/镜）/ 慢镜强调（2–3s/镜）/ 变速节奏（混合） |
| CTA 形式 | 立即下单 / 点击购物车 / 限时优惠 / 品牌心智 / 免费试用 / 悬念引导 / 从众效应 |
...
> 所有事务限制在单实例内，禁止跨服务分布式事务（FastAPI Worker 不参与 PG 事务）。
...
1. **统一 `viral_genes.category` 枚举**：从中文迁移到英文 slug（与 6.1 / API 附录 A 对齐）。
2. **`scripts.storyboard` 顶层结构化**：从「裸数组」升级为 `{factors, shots}` 双键对象。
